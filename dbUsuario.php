...
if (filter_has_var(INPUT_POST, 'btnLogar')) {
    ...
    if (!empty($resultado)) {
        ...
        if (password_verify($senha, $u->senha)) {
            ...
            $_SESSION['nomeUsuario'] = $u->usuario ?? $u->nomeCompleto;
...
if (filter_has_var(INPUT_POST, "btnGravar")) {
    ...
    echo "<script>alert('Cadastro realizado com sucesso!');location.href='usuario.php';</script>";
    exit;
}
...
if (filter_has_var(INPUT_GET, "acao") && filter_input(INPUT_GET, "acao") === "deletar") {
    ...
    if ($u && $u->perfil === 'admin') {
        echo "<script>alert('Não é permitido excluir um administrador.');location.href='usuario.php';</script>";
        exit;
    }

    $Usuario->delete("id", $id);
    echo "<script>alert('Usuário excluído com sucesso!');location.href='usuario.php';</script>";
    exit;
}
